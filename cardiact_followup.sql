...
-- Step 2: Flag risk behaviors based on procedure codes and service dates for cardiac
risk_flags AS (
  SELECT c.MembershipNumber AS member_id,
    -- Flag 1: No cardiac check (e.g., ECG 93000/93005) in last 30 days
    MAX(CASE WHEN (c.PROC_CD IN ('93000', '93005') OR c.PROC_DESC LIKE '%ecg%') THEN c.PROC_DT ELSE NULL END) < CURRENT_DATE - INTERVAL '30' DAY AS skip_cardiac_check,
    ...
    -- Flag 3: Recent ER visit (POS 23 or cardiac ER codes) in last 35 days
    EXISTS (
      SELECT 1
...
final_list AS (
  SELECT member_id,
    (COALESCE(skip_cardiac_check::int, 0) + COALESCE(miss_clinic::int, 0) + COALESCE(recent_er::int, 0)) AS risk_score,
    CASE WHEN skip_cardiac_check THEN 'Y' ELSE 'N' END AS skip_cardiac,
    CASE WHEN miss_clinic THEN 'Y' ELSE 'N' END AS miss_clinic,
    CASE WHEN recent_er THEN 'Y' ELSE 'N' END AS recent_er
  FROM risk_flags
  WHERE (COALESCE(skip_cardiac_check::int, 0) + COALESCE(miss_clinic::int, 0) + COALESCE(recent_er::int, 0)) >= 2
)

-- Output: Top 500 high-risk cardiac members to recommend for C-SNP shift
SELECT member_id, risk_score, skip_cardiac, miss_clinic, recent_er
FROM final_list
