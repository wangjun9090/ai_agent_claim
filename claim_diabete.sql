...
SELECT
    ...
    -- Diabetes flag (fixed - same row, no description-only rule)
    MAX(CASE
        WHEN EXISTS (
            SELECT 1
    ...
    -- Cardiac flag (fixed - same row, no description-only rule)
    MAX(CASE
        WHEN EXISTS (
            SELECT 1
            FROM hive_metastore.off_orig.claims_member sub
            WHERE sub.CLAIM_ID = main.CLAIM_ID
            AND sub.PROC_CD IN (
                '83880', '93306', '93350', 'J2785', '93000', '93005', '93010', '93018'
            )
            AND (
    ...
    -- COPD flag (fixed - same row, no description-only rule)
    MAX(CASE
        WHEN EXISTS (
            SELECT 1
            FROM hive_metastore.off_orig.claims_member sub
